...
export const VISUAL_INFO: Record<string, string> = {
  ...
  agent_performance: "Per-agent resolved count, plus current open and on-hold load. Avg h counts only the time a ticket was assigned to that agent (hold time excluded) — not its full lifetime, so inheriting an old ticket doesn't skew the average. It covers just the tickets in Resolved; work on tickets later passed to someone else is not averaged in. Breaching lists that agent's open tickets already past the SLA (age minus hold time) — click a chip to open the ticket; +Nh is how far past SLA it is. The Avg Resolution KPI above still shows total time from creation to close.",
